...
import { ContainerKind, RouteConstants, convocatoriaDetailPath, encuestaDetailPath, formBuilderPath } from '../../../../core/constants/route.constants';
...
export class FormPreviewComponent implements OnInit {
  ...
  protected goBack(): void {
    ...
    if (tab) {
      const detailPath = kind === 'encuestas' ? encuestaDetailPath(containerId) : convocatoriaDetailPath(containerId);
      this.router.navigate(detailPath, { queryParams: { [RouteConstants.QUERY_TAB]: tab } });
      return;
    }

    // Opened from within the builder — the container travels in the route, so returning
    // there keeps its context with no query param to carry.
    this.router.navigate(formBuilderPath(kind, containerId, id));
  }
}
